...
export async function POST(req: NextRequest) {
  try {
    ...
    // `actualRecipient` = where Gmail actually delivers
    ...
    try {
      ...
      return NextResponse.json({
        success: false,
        error:
          "No Gmail sender configured. Connect Gmail from the dashboard first.",
        detail: String(err),
      });
    }
    ...
    try {
      ...
      const trackingPayload = {
        ...
        // TEST_EMAIL_OVERRIDE target. The actual Gmail send went to
        // `actualRecipient`; the dashboard wants to see who it was
        // intended for.
        to_email: to,
